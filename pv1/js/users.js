...
function login() {
    ...
    let contxt = document.getElementById("cont-txt")
    ...
    if (user === 'mil@mil.com' && pass === '123') {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            contxt.innerHTML = `Tu saldo actual es: ${saldo}`;
        }); 
        function depositar(){
            let sum = Number (prompt ('¿Cuanto vas a depositar?'));
            let saldoActual = saldo + sum;
            document.getElementById("cont-txt").innerHTML=`Tu saldo actual es ${saldoActual}`;
        }
        function retirar (){
